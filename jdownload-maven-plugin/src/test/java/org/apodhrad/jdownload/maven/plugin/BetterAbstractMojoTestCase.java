...
/**
 * Source: http://stackoverflow.com/questions/9496534/test-default-values-and-expressions-of-mojos-using-maven-plugin-testing-harness
 * 
 * Use this as you would {@link AbstractMojoTestCase}, where you want more of the standard maven defaults to be set (and
 * where the {@link AbstractMojoTestCase} leaves them as null or empty). This includes: <li>local repo, repo sessions
 * and managers configured <li>maven default remote repos installed (NB: this does not use your ~/.m2 local settings)
 * <li>
 * system properties are copies
 * <p>
 * No changes to subclass code is needed; this simply intercepts the {@link #newMavenSession(MavenProject)} method used
 * by the various {@link #lookupMojo(String, File)} methods.
 * <p>
 * This also provides new methods, {@link #newMavenSession()} to conveniently create a maven session, and
 * {@link #lookupConfiguredMojo(File, String)} so you don't have to always build the project yourself.
 */
public abstract class BetterAbstractMojoTestCase extends AbstractMojoTestCase {
	...
	/**
	 * As {@link #lookupConfiguredMojo(MavenProject, String)} but taking the pom file and creating the
	 * {@link MavenProject}.
	 */
